...
// keepToss: KEEP
// reasoning: This specification requires understanding sequence operations, string indexing, and the relationship between input and output lengths, making it a good test of implementation skills.
...
// keepToss: KEEP
// reasoning: The specification involves complex matrix-vector multiplication with concurrent processes, requiring sophisticated reasoning about 2D arrays, sequences, invariants, and process coordination - sufficiently challenging for benchmark inclusion.
...
// keepToss: KEEP
// reasoning: This specification requires implementing binary search on a sorted array, which is a classic algorithmic problem that meaningfully tests understanding of loop invariants, bounds checking, and efficient search strategies.
...
// keepToss: KEEP
// reasoning: Complex interval operations requiring understanding of mathematical properties like overlap, intersection, union with preconditions, and proving relationships between predicates - substantial implementation challenge.
...
// keepToss: KEEP
// reasoning: This specification requires implementing multiple non-trivial functions (length, mem, at) and a method that converts arrays to lists while maintaining complex correctness properties including length preservation, element correspondence, and membership relationships.
...
// keepToss: KEEP
// reasoning: This specification requires understanding array manipulation, proper handling of edge cases (single element arrays), and correct swapping logic with precise postconditions about unchanged middle elements.
...
// keepToss: KEEP
// reasoning: This specification provides a meaningful sorting problem with helper functions, proper preconditions/postconditions including sortedness and permutation preservation, requiring non-trivial algorithmic reasoning to implement correctly.
...
// keepToss: KEEP
// reasoning: This specification requires finding the longest common prefix between two strings, which involves non-trivial logic to iterate and compare characters while maintaining correctness properties about the result length and character matching.
...
// keepToss: KEEP
// reasoning: The specification requires implementing array rotation with modular arithmetic, which involves non-trivial index calculations and proper handling of wraparound logic.
...
// keepToss: KEEP
// reasoning: This is a comprehensive priority queue specification using Braun trees with complex invariants (binary heap property and balance), non-trivial operations like DeleteMin and ReplaceRoot, and sophisticated correctness proofs involving multiset reasoning - it would meaningfully test a model's ability to implement data structure operations while maintaining multiple invariants.
...
// keepToss: KEEP
// reasoning: This specification requires understanding array indexing, sequence operations, and the relationship between array slicing and sequence concatenation, making it a meaningful test of implementation skills.
...
// keepToss: KEEP
// reasoning: The specification requires implementing a predicate to identify specific characters and a string transformation method with precise character-by-character correspondence rules, testing both predicate logic and string manipulation skills.
...
// keepToss: KEEP
// reasoning: This specification represents the classic two-sum problem with non-trivial constraints that require implementing an efficient algorithm with proper index ordering and ensuring the first valid pair is returned.
...
// keepToss: KEEP
// reasoning: The specification defines a complex circuit representation system with non-trivial operations like combining circuits, managing port connections, and maintaining wellformedness invariants across transformations, requiring sophisticated reasoning about data structure manipulation and invariant preservation.
...
// keepToss: KEEP
// reasoning: This specification requires finding the maximum in a prefix while also computing an online algorithm result, involving complex logical reasoning about array bounds, quantifiers, and conditional guarantees.
...
// keepToss: TOSS
// reasoning: Postcondition is impossible to satisfy for arrays containing odd numbers, making this an unsolvable specification rather than a meaningful programming challenge.
...
// keepToss: TOSS
// reasoning: These are proof-only lemmas with empty bodies, not function specifications that require implementation - they test proof automation rather than programming ability.
/*
...
// keepToss: TOSS
// reasoning: This specification contains only abstract predicates and empty method bodies with no meaningful contracts, postconditions, or logical constraints that would require non-trivial implementation reasoning.
...
// keepToss: TOSS
// reasoning: The specification is too simple - finding the maximum element in an array is a basic programming exercise that doesn't meaningfully test advanced reasoning abilities, and the incomplete maxcheck function lacks proper specification.
...
// keepToss: TOSS
// reasoning: The specification is trivial with an empty postcondition - it requires no meaningful implementation since any code that doesn't crash would satisfy the spec, making it unsuitable for testing a model's ability to implement sorting logic.
// Sorting: 
...
// keepToss: TOSS
// reasoning: This specification contains only trivial functions and empty methods with no meaningful specifications to implement.
...
// keepToss: TOSS
// reasoning: The specification is too simple - it's just a straightforward universal quantifier check that can be implemented with a basic loop, requiring minimal reasoning about the relationship between the ensures clauses.
...
// keepToss: TOSS
// reasoning: This is a trivial specification that directly maps to Dafny's built-in `in` operator, requiring no meaningful algorithmic thinking or implementation complexity.
...
// keepToss: TOSS
// reasoning: The specification contains many functions with empty bodies and lemmas that lack meaningful constraints, making them too trivial to test implementation skills effectively.
...
// keepToss: TOSS
// reasoning: This is a trivial logic puzzle that only requires instantiating a universal quantifier with x=0, offering no meaningful programming challenge.
predicate P(x: int)
...
// keepToss: TOSS
// reasoning: The specification is incomplete and trivial - the vowels set is empty, making the function meaningless, and there are no meaningful postconditions or constraints that would challenge implementation.
...
// keepToss: TOSS
// reasoning: Functions are too simple (basic arithmetic operations) and predicates lack meaningful specifications - IsOddNat, IsEvenNat, and IsPrime are empty bodies that would trivially test basic modular arithmetic and divisibility concepts.
...
// keepToss: TOSS
// reasoning: This is a trivial arithmetic calculation that directly implements the mathematical formula without any algorithmic complexity or reasoning challenges.
...
// keepToss: TOSS
// reasoning: The helper predicates is_sorted and is_sorted_and_distinct are empty, making the specification incomplete and impossible to implement correctly without first defining these essential predicates.
...
// keepToss: TOSS
// reasoning: The specification is incomplete as the Power function has no postcondition defining what power operation it should perform, making it impossible to implement meaningfully.
