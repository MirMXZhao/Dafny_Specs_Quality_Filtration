...
method flatten2(mat: array2<int>) returns (ret: array<int>)
    ...
    ensures forall i, j :: 0 <= i < mat.Length0 && 0 <= j < mat.Length1 ==> i*mat.Length1 + j <= (mat.Length0 - 1) * mat.Length1 + mat.Length1 -1;
...
{ //I don't understand why it's not verifying :(
        ...
        //ensures forall i, j :: 0 <= i < mat.Length0 && 0 <= j < mat.Length1 ==> ret[i * mat.Length1 + j] == mat[i, j];
    {
        ...
        {   
            ...
            ret[i*mat.Length1 + j] := mat[i, j]; // should be verified at this point? 
            j:= j+1;
        }
        i := i+1; 
    }
