...
method TestCombineBackconns1() {
  var offset := 3;
  var bc1 := map[inodeport(0, 0) := onodeport(1, 0), inodeport(2, 1) := onodeport(0, 2)];
  var bc2 := map[inodeport(0, 0) := onodeport(1, 1), inodeport(1, 0) := onodeport(0, 0)];
  var result := BackwardConnections.CombineBackconns(offset, bc1, bc2);
  assert result == map[inodeport(0, 0) := onodeport(1, 0), inodeport(2, 1) := onodeport(0, 2), inodeport(3, 0) := onodeport(4, 1), inodeport(4, 0) := onodeport(3, 0)];
}

method TestCombineBackconns2() {
  var offset := 2;
  var bc1 := map[inodeport(1, 0) := onodeport(0, 1)];
  var bc2 := map[inodeport(0, 1) := onodeport(2, 0)];
  var result := BackwardConnections.CombineBackconns(offset, bc1, bc2);
  assert result == map[inodeport(1, 0) := onodeport(0, 1), inodeport(2, 1) := onodeport(4, 0)];
}
