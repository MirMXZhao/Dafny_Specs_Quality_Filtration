...
method TestInsert1() {
    var pq := PQ.Empty();
    var pq' := PQ.Insert(pq, 5);
    assert pq' == PQ.Node(5, PQ.Leaf, PQ.Leaf);
}

method TestInsert2() {
    var pq := PQ.Node(3, PQ.Leaf, PQ.Leaf);
    var pq' := PQ.Insert(pq, 1);
    assert pq' == PQ.Node(1, PQ.Node(3, PQ.Leaf, PQ.Leaf), PQ.Leaf);
}
