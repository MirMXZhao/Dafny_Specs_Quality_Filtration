...
method TestConcat1() {
    var r1 := new Rope.Terminal("Hello");
    var r2 := new Rope.Terminal("World");
    var result := Rope.concat(r1, r2);
    assert result != null;
    assert result.Valid();
    assert result.Contents == "HelloWorld";
    assert result.left == r1;
    assert result.right == r2;
}

method TestConcat2() {
    var r1 := new Rope.Terminal("Test");
    var result := Rope.concat(r1, null);
    assert result == r1;
    assert result != null;
    assert result.Valid();
    assert result.Contents == "Test";
}
