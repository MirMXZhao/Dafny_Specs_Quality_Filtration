...
method TestHasValidJumpTargets1() {
  var ss := [JmpZero(R0, 2), Add(R0, R1, R2), Halt];
  var result := has_valid_jump_targets(ss, 0);
  assert result == true;
}

method TestHasValidJumpTargets2() {
  var ss := [JmpZero(R0, 5), Add(R0, R1, R2)];
  var result := has_valid_jump_targets(ss, 0);
  assert result == false;
}
