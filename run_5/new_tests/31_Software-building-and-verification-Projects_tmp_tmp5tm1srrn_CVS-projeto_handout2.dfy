...
method TestHashtable1() {
  var ht := new Hashtable<int, string>(5);
  ht.add(10, "ten");
  var result := ht.find(10);
  assert result == Some("ten");
}

method TestHashtable2() {
  var ht := new Hashtable<string, int>(3);
  ht.add("key1", 100);
  ht.add("key2", 200);
  var result1 := ht.find("key1");
  var result2 := ht.find("key3");
  assert result1 == Some(100);
  assert result2 == None;
}
