...
method TestNextStepDeterministicGivenStep1() {
  var v := Variables([0, 1, 2], [-1, -1, -1]);
  var step := TransmissionStep(0);
  var v'1 := Variables([0, 1, 2], [-1, 0, -1]);
  var v'2 := Variables([0, 1, 2], [-1, 0, -1]);
  NextStepDeterministicGivenStep(v, step, v'1, v'2);
  assert v'1 == v'2;
}

method TestNextStepDeterministicGivenStep2() {
  var v := Variables([5, 3, 8], [5, 8, 3]);
  var step := TransmissionStep(1);
  var v'1 := Variables([5, 3, 8], [5, 8, 8]);
  var v'2 := Variables([5, 3, 8], [5, 8, 8]);
  NextStepDeterministicGivenStep(v, step, v'1, v'2);
  assert v'1 == v'2;
}
