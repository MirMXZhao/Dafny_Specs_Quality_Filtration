...
method TestGetAllShuffledDataEntries1() {
  var input := new int[3];
  input[0] := 1;
  input[1] := 2;
  input[2] := 3;
  var result := getAllShuffledDataEntries(input);
  assert result.Length == 3;
  assert multiset(result[..]) == multiset([1, 2, 3]);
}

method TestGetAllShuffledDataEntries2() {
  var input := new string[2];
  input[0] := "hello";
  input[1] := "world";
  var result := getAllShuffledDataEntries(input);
  assert result.Length == 2;
  assert multiset(result[..]) == multiset(["hello", "world"]);
}
