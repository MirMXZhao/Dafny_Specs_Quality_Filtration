...
method TestCombineBackconns1() {
    var bc1 := map[inodeport(0, 1) := onodeport(2, 0)];
    var bc2 := map[inodeport(1, 0) := onodeport(2, 1)];
    var result := BackwardConnections.CombineBackconns(3, bc1, bc2);
    assert result == map[inodeport(0, 1) := onodeport(2, 0), inodeport(4, 0) := onodeport(5, 1)];
}

method TestCombineBackconns2() {
    var bc1 := map[inodeport(1, 2) := onodeport(0, 1), inodeport(2, 0) := onodeport(1, 2)];
    var bc2 := map[inodeport(0, 1) := onodeport(1, 0)];
    var result := BackwardConnections.CombineBackconns(5, bc1, bc2);
    assert result == map[inodeport(1, 2) := onodeport(0, 1), inodeport(2, 0) := onodeport(1, 2), inodeport(5, 1) := onodeport(6, 0)];
}
