method LinearSearch<T>(a: array<T>, P: T -> bool) returns (n: int)
    ensures -1 <= n < a.Length
    ensures n == -1 || P(a[n])
    ensures n != -1 ==> forall i :: 0 <= i < n ==> ! P(a[i])
    ensures n == -1 ==> forall i :: 0 <= i < a.Length ==> ! P(a[i])
{}

method LinearSearch1<T>(a: array<T>, P: T -> bool, s1:seq<T>) returns (n: int)
    requires |s1| <= a.Length
    requires forall i:: 0<= i <|s1| ==> s1[i] == a[i]
    ensures -1 <= n < a.Length
    ensures n == -1 || P(a[n])
    ensures n != -1 ==> forall i :: 0 <= i < n ==> ! P(a[i])
    ensures n == -1 ==> forall i :: 0 <= i < |s1| ==> ! P(a[i])
{}


method LinearSearch2<T(==)>(data: array<T>, Element:T, s1:seq<T>) returns (position:int)
    requires |s1| <= data.Length
    requires forall i:: 0<= i <|s1| ==> s1[i] == data[i]
    ensures position == -1 || position >= 1
    ensures position >= 1 ==> exists i::0 <=i < |s1| && s1[i] == Element
    ensures position == -1 ==> forall i :: 0 <= i < |s1| ==> s1[i] != Element
{}

method LinearSearch3<T(==)>(data: array<T>, Element:T, s1:seq<T>) returns (position:int)
    requires |s1| <= data.Length
    requires forall i:: 0<= i <|s1| ==> s1[i] == data[data.Length -1-i]
    ensures position == -1 || position >= 1
    ensures position >= 1 ==> exists i::0 <=i < |s1| && s1[i] == Element && |s1| != 0
{}

////////TESTS////////

method TestLinearSearch1() {
  var a := new int[4] [1, 2, 3, 4];
  var n := LinearSearch(a, x => x == 3);
  assert n == 2;
}

method TestLinearSearch2() {
  var a := new int[3] [1, 2, 4];
  var n := LinearSearch(a, x => x == 5);
  assert n == -1;
}

method TestLinearSearch11() {
  var a := new int[4] [1, 2, 3, 4];
  var s1 := [1, 2];
  var n := LinearSearch1(a, x => x == 2, s1);
  assert n == 1;
}

method TestLinearSearch12() {
  var a := new int[4] [1, 2, 3, 4];
  var s1 := [1, 2];
  var n := LinearSearch1(a, x => x == 5, s1);
  assert n == -1;
}

method TestLinearSearch21() {
  var data := new int[3] [1, 2, 3];
  var s1 := [1, 2];
  var position := LinearSearch2(data, 2, s1);
  assert position == 1;
}

method TestLinearSearch22() {
  var data := new int[3] [1, 2, 3];
  var s1 := [1, 2];
  var position := LinearSearch2(data, 5, s1);
  assert position == -1;
}

method TestLinearSearch31() {
  var data := new int[3] [1, 2, 3];
  var s1 := [3, 2];
  var position := LinearSearch3(data, 3, s1);
  assert position == 1;
}

method TestLinearSearch32() {
  var data := new int[3] [1, 2, 3];
  var s1 := [3, 2];
  var position := LinearSearch3(data, 5, s1);
  assert position == -1;
}
