...
method TestNonOverlappingIntervals1() {
  var intervals := new int[3, 2];
  intervals[0, 0] := 1; intervals[0, 1] := 2;
  intervals[1, 0] := 2; intervals[1, 1] := 3;
  intervals[2, 0] := 3; intervals[2, 1] := 4;
  var count := non_overlapping_intervals(intervals);
  assert count == 0;
}

method TestNonOverlappingIntervals2() {
  var intervals := new int[3, 2];
  intervals[0, 0] := 1; intervals[0, 1] := 3;
  intervals[1, 0] := 2; intervals[1, 1] := 4;
  intervals[2, 0] := 3; intervals[2, 1] := 5;
  var count := non_overlapping_intervals(intervals);
  assert count == 1;
}
