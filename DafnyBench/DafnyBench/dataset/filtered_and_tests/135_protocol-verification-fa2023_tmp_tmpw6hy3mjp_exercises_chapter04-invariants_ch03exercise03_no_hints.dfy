...
method TestClientHoldsLock1() {
  var v := Variables(3, Client(1), [Released, Acquired, Released]);
  var result := ClientHoldsLock(v, 1);
  assert result == true;
}

method TestClientHoldsLock2() {
  var v := Variables(3, Unlocked, [Released, Released, Released]);
  var result := ClientHoldsLock(v, 0);
  assert result == false;
}
