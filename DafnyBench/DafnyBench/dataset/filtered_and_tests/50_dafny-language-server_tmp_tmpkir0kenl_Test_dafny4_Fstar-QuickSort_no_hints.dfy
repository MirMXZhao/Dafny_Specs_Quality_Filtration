...
method TestSort1() {
  var input := Cons(3, Cons(1, Cons(4, Cons(2, Nil))));
  var result := sort(1, 4, input);
  assert SortedRange(1, 4, result);
  assert forall x :: In(x, input) == In(x, result);
}

method TestSort2() {
  var input := Cons(5, Nil);
  var result := sort(5, 5, input);
