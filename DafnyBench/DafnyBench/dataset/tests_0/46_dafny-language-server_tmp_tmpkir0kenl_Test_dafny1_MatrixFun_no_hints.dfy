...
method TestMirrorImage2() {
  ...
  m[2,0] := 5; m[2,1] := 6;
  MirrorImage(m);
  assert m[0,0] == 2 && m[0,1] == 1;
  assert m[1,0] == 4 && m[1,1] == 3;
  assert m[2,0] == 6 && m[2,1] == 5;
}

method TestMirrorImage3() {
  var m := new int[1,1];
  m[0,0] := 42;
  MirrorImage(m);
  assert m[0,0] == 42;
...
method TestPrintMatrix2() {
  var m := new int[1,3];
  m[0,0] := 5; m[0,1] := 6; m[0,2] := 7;
  PrintMatrix(m);
}

method TestPrintMatrix3() {
  var m := new int[0,0];
  PrintMatrix(m);
}
