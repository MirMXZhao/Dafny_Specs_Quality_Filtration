...
def filter_problem_concurrent(data, prompts, filtered, maxTokens=100, sheetName = 'sheet2', max_workers=10):
        ...
        try:
            response = client.beta.messages.create(
                ...
                system=[
                    {
                        "type": "text",
                        "text": prompts["filter_problems_2"]["overall_goal"] + prompts["filter_problems_2"]["task"] + prompts["filter_problems_2"]["examples"],
                ...
                messages=[
                    {
                        ...
                        "content": [
                            {
                                "type": "text",
                                "text": prompts["filter_problems_2"]["output_request"] + prompts["filter_problems_2"]["file"],
                                "cache_control": {"type": "ephemeral"}
                            },
                            {
                                "type": "document",
    ...
    print(f"Processing complete! Results saved to filterProblems2.xlsx")
    return result
