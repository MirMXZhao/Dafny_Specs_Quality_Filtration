...
def intermediate_filter(data):
    ...
    for i in range(len(prevFilter["filename"])):
        if prevFilter["keepTrash"][i] == "KEEP":
            prevKeep.append(prevFilter["index"][i])
    return prevKeep
...
def filter_problem_concurrent(data, prompts, filtered, maxTokens=100, sheetName = 'sheet2', max_workers=10):
        ...
        try:
            response = client.beta.messages.create(
                ...
                system=[
                    {
                        "type": "text",
                        "text": prompts["filter_problems_3"]["overall_goal"] + prompts["filter_problems_3"]["task"] + prompts["filter_problems_3"]["examples"],
                ...
                messages=[
                    {
                        ...
                        "content": [
                            {
                                "type": "text",
                                "text": prompts["filter_problems_3"]["output_request"] + prompts["filter_problems_3"]["file"],
                                "cache_control": {"type": "ephemeral"}
                            },
                            {
                                "type": "document",
            ...
            keep_trash, reasoning = text.splitlines()
            ...
            return (i, filename, fileid, keep_trash, reasoning)
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(process_single_file, i) for i in filtered if i < 30]
        ...
        for future in futures:
            i, filename, fileid, keep_trash, reasoning = future.result()
    ...
    df_result.to_excel('/Users/cinnabon/Documents/MIT/UROP_2025/DafnyBench/filtration_ai/filterProblems3.xlsx', sheet_name=sheetName, index=False)
    
    print(f"Processing complete! Results saved to filterProblems3.xlsx")
    return result
...
if __name__ == "__main__":
    ...
    print(len(filtered))
    result = filter_problem_concurrent(data, prompts, filtered, maxTokens=100, sheetName = 'Sheet1', max_workers=5)
